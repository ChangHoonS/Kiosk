package practice.save.savapacka;
...
public class Kiosk {
    ...
    Kiosk(List<Menu> menu, List<MenuItem> burgers, List<MenuItem> setMenu, List<MenuItem> drinks, List<MenuItem> sides) {
        this.menu = menu;
            ...
            for (int i = 1; i <= menu.size(); i++) {
                if (choice == 1) {
                    ...
                    for (int j = 1; j <= burgers.size(); j++) {
                        System.out.println(j + ". " + burgers.get(j - 1).name + " | W " + burgers.get(j - 1).price + " | " + burgers.get(j - 1).explanation);
                    }
